...
def run_test_polygon():
    ...
    title = 'POLYGON test 3:  10 segments with medium purple lines.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(200, 300), 100)
    circle.outline_thickness = 3
    polygon(window, circle, 10, 'purple', 4)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(points)):
        if points[k] == points[len(points) - 1]:
            segment = rg.Line(points[len(points) - 1], points[0])
        else:
            segment = rg.Line(points[k], points[k + 1])
        segment.color = color
...
def run_test_fancy_polygon():
    ...
    # Test 4
    title = 'FANCY POLYGON test 4:  300 red lines on grey circle, hops = 61.'
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(points)):
        if points[k] == points[len(points) - 1]:
            line = rg.Line(points[len(points) - 1], points[(hops_to_next_point % len(points)) - 1])
        else:
